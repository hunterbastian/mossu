...
interface CameraColliderOptions {
  fadeWhenOccluding?: boolean;
}

export function markCameraCollider(mesh: Mesh, options: CameraColliderOptions = {}) {
  mesh.userData.cameraCollider = true;
  if (options.fadeWhenOccluding) {
    mesh.userData.cameraOccluderFade = true;
  }
  return mesh;
}

export function scatterAroundPocket(pocket: { position: Vector3; radius: number }, index: number, radiusScale = 1) {
  const angleSeed =
    Math.sin((index + 1) * 12.9898 + pocket.position.x * 0.013 + pocket.position.z * 0.019) * 43758.5453;

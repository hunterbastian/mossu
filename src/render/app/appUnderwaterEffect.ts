...
export function getUnderwaterEffectTargetIntensity(camera: Camera, player: PlayerState) {
  ...
  const cameraIntensity =
    cameraWater && depthBelowSurface > 0.08 ? MathUtils.clamp(0.22 + depthBelowSurface / 8.5, 0, 1) : 0;
  const playerIntensity =
    player.waterMode === "underwater" ? MathUtils.clamp(0.38 + player.waterDepth / 10, 0.38, 0.82) : 0;

# Agent setup for `mossu`

This is a deepsec scanning workspace. Project `mossu` was just registered
(target: `..`). Setup is incomplete — `data/mossu/INFO.md`
...
2. **Fill in `data/mossu/INFO.md`.** It's auto-injected into the AI
...
```bash
pnpm deepsec scan    --project-id mossu
pnpm deepsec process --project-id mossu
```

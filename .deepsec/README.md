...
Currently configured project: `mossu` (target: `..`).
...
3. Open the parent repo in your coding agent (Claude Code, Cursor, …)
   and have it follow `data/mossu/SETUP.md` to fill in
   `data/mossu/INFO.md`.
...
`--project-id mossu` (or whichever id you want) explicitly.

`scan` is free (regex only). `process` is the AI stage (≈$0.30/file
on Opus by default). Run state goes to `data/mossu/`.

...
- sky/backdrop references should be translated as distant-only atmosphere: deep cyan upper sky, pale turquoise ocean haze, giant soft cumulus cloud banks, far floating islands, tiny hazed silhouettes, faint birds, and sparse sun/ocean sparkles. Keep nearby skybox props out of this layer so the illusion does not break when Mossu moves.
...
- Water runtime ownership is intentionally split: `waterSystem.ts` facade, `waterBodies.ts` placement/group construction, `waterSurfaceFactory.ts` controller assembly, `waterSurfaceShader.ts` main surface shader hook, `waterVolumeShader.ts` volume shader hook, `waterFillShader.ts` fill/underfill shader hook, `waterShaderChunks.ts` shared wave/ripple GLSL snippets, `waterSurfaceGeometry.ts` mesh attributes/underfill, `waterSurfaceLayers.ts` fill/volume layer mesh construction, `waterSurfaceController.ts` lifecycle/update/dispose, `waterSceneUniforms.ts` scene-lighting registration, `waterSurfaceColors.ts` profile color setup, `waterSurfaceMaterials.ts` material construction, `waterProfileUniforms.ts` scalar profile uniform setup, `waterShaderUniforms.ts` shared wave/ripple uniforms, `waterProfiles.ts` tuning knobs, `waterRipples.ts` actor disturbance signals, `waterWaterfalls.ts` decorative cascade accents, and `waterTypes.ts` shared interfaces. Keep future passes inside the narrow owner when possible.
...
- Distant floating islands should sit on a broad horizon/ocean arc, with closer readable islands still far away and smaller haze-tier islands reduced to pale silhouettes. Use low-poly, fog-friendly forms and broad spacing rather than dense nearby sky clutter.
...
- The far ocean should grade by distance: rich blue near the camera, turquoise-blue through the middle distance, pale cyan toward the far plane, then cream/peach haze at the horizon so the floating island sits in humid open air. The current open-ocean target is brighter tropical adventure water with a Sea-of-Thieves-like motion read and the latest close-up wave reference: taller curling Gerstner breakers, bright turquoise wave faces, darker trough contrast, slope-aware sun glints, long broken white foam lace, and crest-spray speckles tied to wave energy rather than static noise.

...
- **`main`** dynamically imports **`GameApp`** (full game), **`ModelViewerApp`** (`?modelViewer`), or **`IslandViewerApp`** (`?islandViewer`).
...
| **`visualProbe=1`**                           | Lets deterministic `advanceTime()` calls render canvas frames in e2e mode and preserves the WebGL drawing buffer for visual probes. `npm run art:review` uses `?qaDebug=1&visualProbe=1`, the normal headed browser render loop, and async canvas PNG capture.                                      |
| **`spiritCloseup=1`**                         | Debug framing for the spirit / closeup rig (passed into `WorldRenderer`).                                                                                                                                                                                                                           |
| **`modelViewer=1`** or **`modelViewer=karu`** | Loads the isolated **ModelViewerApp** chunk instead of the game; `modelViewer=karu` opens directly on the Karu companion rig. The viewer uses the same neutral debug-tool chrome as the island atlas.                                                                                              |
| **`islandViewer=1`**                          | Loads the isolated **IslandViewerApp** debug atlas instead of gameplay. Use it to orbit or fly the full floating island, switch overview/top/profile/under presets, inspect the named Terrain/Water/Forests/Meadows/Rocks/Landmarks/Lighting/Debug folders, toggle terrain/biomes/rocks/ocean/water/guides/landmarks/falls layers, and confirm the ocean-below read without changing the player camera. |
...
- **Art-review compare**: `npm run art:compare` validates `output/art-review-route/summary.json`, associated JSON files, screenshots, and PNG visual metrics so incomplete, stale, blank, or repeated sky-only captures fail. Pass `-- --baseline path/to/summary.json` to compare route state against a saved baseline.

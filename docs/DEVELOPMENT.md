...
| **`visualProbe=1`**                           | Lets deterministic `advanceTime()` calls render canvas frames in e2e mode; used by targeted visual probes. `npm run art:review` now uses `?qaDebug=1` and the normal headed browser render loop instead.                                                                                             |
| **`spiritCloseup=1`**                         | Debug framing for the spirit / closeup rig (passed into `WorldRenderer`).                                                                                                                                                                                                                           |
| **`modelViewer=1`** or **`modelViewer=karu`** | Loads the isolated **ModelViewerApp** chunk instead of the game; `modelViewer=karu` opens directly on the Karu companion rig.                                                                                                                                                                       |
